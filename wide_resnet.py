...
class BasicWideBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if groups != 1 or base_width != 128:
            raise ValueError('BasicWideBlock only supports groups=1 and base_width=128')
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicWideBlock")
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if zero_init_residual:
            for m in self.modules():
                ...
                elif isinstance(m, BasicWideBlock):
                    nn.init.constant_(m.bn2.weight, 0)
...
def wide_resnet18_2(pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('wide_resnet18_2', BasicWideBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
